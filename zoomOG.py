...
class CanvasImage:
    ...
    def __wheel(self, event):
        ...
        if event.num == 5 or event.delta == -120:  # scroll down, smaller
            if (self.imscale <= 1): return  # image is less than 30 pixels
            self.imscale /= self.__delta
            scale        /= self.__delta
        if event.num == 4 or event.delta == 120:  # scroll up, bigger
            if (self.imscale >= 10): return  # 1 pixel is bigger than the visible area
            self.imscale = round(self.imscale*self.__delta,1)
            scale        = round(scale*self.__delta,1)
        if (self.imscale <= 1):
            self.imscale = 1
            scale = 1
...
class MainWindow(ttk.Frame):
    """ Main window class """
    def __init__(self, mainframe, path, posX, posY):
        """ Initialize the main Frame """
        ttk.Frame.__init__(self, master=mainframe)
        self.master.title('Map Exporter')
        self.master.geometry('640x640')  # size of the main window
        self.master.rowconfigure(0, weight=1)  # make the CanvasImage widget expandable
        self.master.columnconfigure(0, weight=1)
        canvas = CanvasImage(self.master, path)  # create widget
        canvas.place(row=1, column=0)  # show widget