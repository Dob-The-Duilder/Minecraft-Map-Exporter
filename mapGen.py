...
import time
import gzip, os, itertools, struct, sys
...
def imageCombine(coordsX, coordsZ, imgList, sizList, folder_path):
    ...
    if (np.max(sizList) != np.min(sizList)): 
        return [128,128, imgList[0], 'Maps not the same scale']
    scale = sizList[0]
...
#   Listen I got no idea how this stuff works please dont ask me.
